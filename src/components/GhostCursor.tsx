...
const GhostCursor: React.FC<GhostCursorProps> = ({
  ...
  const hasValidSizeRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (pointerActiveRef.current) {
        ...
        // Ambient gentle drift when mouse is still
        const ambientX = 0.5 + Math.sin(t * 0.4) * 0.35 + Math.cos(t * 0.25) * 0.1;
        const ambientY = 0.5 + Math.cos(t * 0.35) * 0.35 + Math.sin(t * 0.15) * 0.1;
        currentMouseRef.current.lerp(new THREE.Vector2(ambientX, ambientY), 0.02);
        mat.uniforms.iMouse.value.copy(currentMouseRef.current);
        fadeOpacityRef.current = 0.85;
      }
    ...
    const onPointerMove = (e: PointerEvent) => {
      const host = containerRef.current;
      const targetEl = host?.parentElement || host;
      if (!targetEl) return;
