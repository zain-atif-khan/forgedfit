    ...
    for file in files:
        if file.endswith('.ts') or file.endswith('.tsx'):
            ...
            with open(path, 'r', encoding='utf-8') as f:
                content = f.read()
                found = re.findall(r'https://images\.unsplash\.com/photo-[a-zA-Z0-9\-]+', content)
                all_urls.update(found)

all_urls = sorted(list(all_urls))
print(f"Found {len(all_urls)} unique images in src/")
...
for root, _, files in os.walk('src'):
    for file in files:
        if file.endswith('.ts') or file.endswith('.tsx'):
            ...
            # Replace each URL with a unique one from our list, but map original -> unique so it doesn't get messed up
            for original_url in all_urls:
                if original_url in content:
                    my_idx = all_urls.index(original_url)
                    if my_idx < len(images):
                        content = content.replace(original_url, images[my_idx])
